...
public class SplashActivity extends BaseActivity {
	...
	private final int TEST_CODE_2 = 101; 
	private final int TEST_CODE_3 = 102;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		request(TEST_CODE_1, false);
		request(TEST_CODE_2, false);
		request(TEST_CODE_3, false);
	}
	...
	public Object doInBackground(int requsetCode) throws HttpException {
		switch(requsetCode){
			case TEST_CODE_1:
				...
				return action.getXmlDemo("assets://Login.xml");
				
			case TEST_CODE_2:
				action = new DemoAction(mContext);
				return action.getJsonDemo("assets://item.json");
	...
	public void onSuccess(int requestCode, Object result) {
		switch(requestCode){
			case TEST_CODE_1:
				if(result != null){
				    XMLResponse bean = (XMLResponse)result;
			...
			case TEST_CODE_2:
				if(result != null){
				    JSONResponse bean = (JSONResponse)result;
				    NLog.e(tag, bean.toString());
				}
				break;
			...
			case TEST_CODE_3:
				if(result != null){
					List<JSONResponse> bean = (ArrayList<JSONResponse>)result;
					NLog.e(tag, bean.toString());
				}
